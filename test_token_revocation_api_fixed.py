...
class TokenRevocationAPITest:
    ...
    def test_single_token_revocation(self):
        ...
        validation_response = self.client.get('/api/v1/core/auth/validate/', 
                                            HTTP_AUTHORIZATION=f'Bearer {tokens["access_token"]}')
    ...
    def test_revoke_all_user_tokens(self):
        ...
        response = self.client.post('/api/v1/core/auth/revoke-all/', {
    ...
    def test_device_token_revocation(self):
        ...
        # Create a fresh user for this test to avoid token conflicts
        email = f'device_test_{uuid.uuid4().hex[:8]}@example.com'
        fresh_user = User.objects.create_user(
            email=email,
            username=email,  # Set username to email to avoid conflicts
            password='testpass123',
            first_name='Device',
            last_name='Test'
        )
        ...
        assert response.status_code == 200, f"Fresh login should succeed, got {response.status_code}"
        tokens = response.json()['tokens']
        
        # Revoke device tokens
        response = self.client.post('/api/v1/core/auth/revoke-device/', {
    ...
    def test_bulk_token_revocation(self):
        ...
        # Bulk revoke tokens
        response = self.client.post('/api/v1/core/auth/bulk-revoke/', {
    ...
    def cleanup(self):
        ...
        try:
            # Don't delete the user since it might be used by other tests
